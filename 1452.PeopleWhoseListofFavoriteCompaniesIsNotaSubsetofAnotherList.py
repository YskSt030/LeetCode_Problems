...
any other list of favorites companies. You must return the indices in increasing order.
...
Person with index=3 has favoriteCompanies[3]=["google"] which is a subset of favoriteCompanies[0]=[
    ...
    def peopleIndexes(self, favoriteCompanies: List[List[str]]) -> List[int]:
        ...
        for i in range(len(fc)):
            ...
            for j in range(len(cs_)):
                if j == 0:
                    set_ = set(clist[cs_[j]])
                    set_.remove(i)
                    if len(cs_) == 1 and len(set_) == 0:
                        ret.append(i)
                else:
                    debug = set(clist[cs_[j]])
                    set_ = set_ & set(clist[cs_[j]])
                    if len(set_) == 0:
                        ret.append(i)
                        break
        return ret
