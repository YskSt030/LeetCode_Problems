...
the 32-bit signed integer range: [−231,  231 − 1]. If the numerical value is out of the 
...
class Solution:
    def myAtoi(self, str: str) -> int:
        ...
        for i in range(len(str)):
            if str[i] == ' ':
                if ret == None and p_m == None:
                    continue
                else:
                    return self.addsign_and_checklim(ret, p_m)
            elif str[i] == '-':
                if ret == None and p_m == None:
                    p_m = True
                else:
                ...
                if ret == None and p_m == None:
                    ...
                    return self.addsign_and_checklim(ret, p_m)
            elif str[i] in num_list:
                if ret is None:
                    ret = num_list.index(str[i])
                else:
                    ret = ret * 10 + num_list.index(str[i])
            else:
                if ret is None:
                    return 0
                else:                    
    ...
    def addsign_and_checklim(self, num_, p_m_):
        INT_MAX = 2**31 - 1
        INT_MIN = -(2**31)
        if num_ == None:
            return 0
        if p_m_ != None and p_m_:
            num_ *= -1
        ret = min(INT_MAX, num_)
        ret = max(INT_MIN, ret)
        return ret
