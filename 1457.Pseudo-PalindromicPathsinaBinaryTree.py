...
the green path [2,1,1], and the path [2,3,1]. Among these paths only red path and green path are 
...
class Solution:
    def pseudoPalindromicPaths (self, root: TreeNode) -> int:
        ...
        self.read_([],root)
        return self.ret
    def read_(self,list_,node):
        l = copy.deepcopy(list_)
        l.append(node.val)
        if node.left == None and node.right == None:            
            ...
            if node.left != None:
                self.read_(l,node.left)
            if node.right != None:
                self.read_(l,node.right)
    
    def check(self,s):
        odds = 0
        evens = 0
        counts = collections.Counter(s)
        tmp = counts.most_common()
        for c in tmp:
            if c[1] % 2 == 0:
                evens += 1
            else:
