...
in case of two or more integers have the same number of 
...
class Solution:
    ...
    def sortByBits(self, arr):
        ...
        for i in range(len(arr)):
            arr_bits.append([i,self.numOfBits(arr[i])])
        arr_bits = sorted(arr_bits, key=lambda x:x[1] )               
        ret = list()
        i = 0
        while i < len(arr):
            bitnum = arr_bits[i][1]
            len_same = len([x for x in arr_bits if x[1] == bitnum])
            if  len_same > 1:
                temp_ = list()
                for j in range(len_same):
                    temp_.append(arr[arr_bits[i+j][0]])
                ret.extend(sorted(temp_))
                i += len_same
            else:
                ret.append(arr[arr_bits[i][0]])
                i += 1
        return ret
