...
(r1, c1) and (r2, c2) is the Manhattan distance, |r1 - r2| + |c1 - c2|.  
...
class Solution:
    def allCellsDistOrder(self, R: int, C: int, r0: int, c0: int) -> List[List[int]]:
        ...
        def getnext(pos):
            ...
            c = pos[1]
            ret = list()
            if r > 0 and [r - 1, c] not in ret:
                ret.append([r - 1, c])
            if r < R and [r + 1, c] not in ret:
                ret.append([r + 1, c])
            if c > 0 and [r, c - 1] not in ret:
                ret.append([r, c - 1])
            if c < C and [r, c + 1] not in ret:
                ret.append([r, c + 1])
        pos = [r0, c0]
        while True:
            prev_num = len(ret)
