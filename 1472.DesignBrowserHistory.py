...
history and steps > x, you will return only x steps. Return the current url after moving back in history at most steps.
...
class BrowserHistory:
    ...
    def visit(self, url: str) -> None:
        if len(self.urls) > self.p + 1:
            self.urls[self.p+1] = url
            self.urls = self.urls[:self.p+2]
            self.p += 1
        else:
    ...
    def back(self, steps: int) -> str:
        ...
        return self.urls[self.p]
        

    def forward(self, steps: int) -> str:
        self.p = min(self.p + steps, len(self.urls)-1)
        return self.urls[self.p]
